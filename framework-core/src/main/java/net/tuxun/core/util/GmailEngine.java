 ...
 * 
 */
public class GmailEngine extends com.octo.captcha.engine.image.ImageCaptchaEngine {
  @SuppressWarnings("unchecked")
  public GmailEngine(WordGenerator wordGen, FontGenerator fontGen, ColorGenerator colerGen,
      BackgroundGenerator backGen, int min, int max, float radius) {
    TextPaster randomPaster =
        new GlyphsPaster(min, max, colerGen, new GlyphsVisitors[] {
            new TranslateGlyphsVerticalRandomVisitor(1), new OverlapGlyphsUsingShapeVisitor(3),
    ...
    word2image =
        new DeformedComposedWordToImage(false, fontGen, backGen, randomPaster,
            new ArrayList<ImageDeformation>(), new ArrayList<ImageDeformation>(), textDef);
    factories.add(new com.octo.captcha.image.gimpy.GimpyFactory(wordGen, word2image, false));
  }

  public GmailEngine() {
    this(new RandomWordGenerator("ABCDEGHJKLMNRSTUWXY235689"), new RandomFontGenerator(40, 40,
        new Font[] {new Font("nyala", Font.BOLD, 40), new Font("Bell MT", Font.PLAIN, 40),
            new Font("Credit valley", Font.BOLD, 40)}, false), new RandomListColorGenerator(
